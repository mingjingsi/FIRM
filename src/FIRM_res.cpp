...
void FIRM_res::loop_by_thread(int res_SS2_ind, int res_tenx_ind){
  ...
  if (arma::all(merge_pair == 999)){
    ...
    Metric(res_SS2_ind, res_tenx_ind) = Metric_PCA;
    ...
    mat integrated_hvg = integrated_scaled.rows(gene_all_hvg_ind-1);
    mat U_integrated;
    vec s_integrated;
    mat V_integrated;
    svd(U_integrated, s_integrated, V_integrated, integrated_hvg.t());
    mat integrated_embedding = U_integrated.cols(0, dims-1) * diagmat(s_integrated.subvec(0, dims-1));

    double Metric_FIRM = mean(MixingMetric(integrated_embedding, dataset_list, k, max_k));

    Metric(res_SS2_ind, res_tenx_ind) = Metric_FIRM;
  }
}
...
void FIRM_res::fit_by_thread(int thread_id){
  while(true){
    ...
    printf("%d %d %d %d\n", thread_id, idx, current_idx_SS2, current_idx_tenx);
    loop_by_thread(current_idx_SS2, current_idx_tenx);
  }
}
