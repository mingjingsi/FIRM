...
// [[Rcpp::export]]
RcppExport SEXP FIRM_res_hvg(arma::mat& SS2, arma::vec& SS2_FindClusters,
                         arma::mat& tenx, arma::vec& tenx_FindClusters,
      ...
      for (int i = 0; i < NNC_tenx.n_cols; i++){
        ...
        if (prop_tenx < prop_SS2){ // If the proportion in 10X is less than that in SS2, subsample cells in the i-th cluster in SS2
          ...
          if (num > 20){ // subsample when the No. of cells is not very small
            ...
            mat SS2_cluster_tmp = SS2.cols(find(SS2_FindClusters == NNC_tenx(tenx_unpaired_ind(j), i)));
            ...
            for (int iter = 0; iter < rept; iter++){
              ...
              mat SS2_all_tmp_new = join_rows(SS2.cols(find(SS2_FindClusters != NNC_tenx(tenx_unpaired_ind(j), i))), SS2_cluster_tmp_new);
              sd_SS2_tmp += stddev(SS2_all_tmp_new, 0, 1);
            }
            vec sd_SS2_tmp_new = sd_SS2_tmp/rept;

            int check = check_merge(SS2, sd_SS2_tmp_new, tenx, dims, SS2_FindClusters, tenx_FindClusters,
          ...
          if (num > 20){ // subsample when the No. of cells is not very small
            // calculate the standard deviation based on the cells after subsampling and scale the data
            mat tenx_cluster_tmp = tenx.cols(find(tenx_FindClusters == tenx_unpaired_ind(j)));
            ...
            for (int iter = 0; iter < rept; iter++){
              ...
              mat tenx_all_tmp_new = join_rows(tenx.cols(find(tenx_FindClusters != tenx_unpaired_ind(j))), tenx_cluster_tmp_new);
              sd_tenx_tmp += stddev(tenx_all_tmp_new, 0, 1);
            }
            vec sd_tenx_tmp_new = sd_tenx_tmp/rept;

            int check = check_merge(SS2, tenx, sd_tenx_tmp_new, dims, SS2_FindClusters, tenx_FindClusters,
                                    NNC_tenx(tenx_unpaired_ind(j), i), tenx_unpaired_ind(j), quantile_def);
  ...
  if (arma::all(merge_pair == 999)){
    ...
    if (n_paired == 1){
      ...
      mat integrated = integrated_scale_fill_hvg(SS2, tenx, sd_SS2, sd_tenx, dims);
